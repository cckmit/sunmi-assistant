...
public class FragmentTabHost extends TabHost implements TabHost.OnTabChangeListener {
    ...
    static class SavedState extends BaseSavedState {
        ...
        public String toString() {
            return "FragmentTabHost.SavedState{"
                    + Integer.toHexString(System.identityHashCode(this)) + " curTab=" + curTab + "}";
        }
     ...
     * {@link #setup(Context, FragmentManager)} or {@link #setup(Context, FragmentManager, int)}.
     */
    @Override
    @Deprecated
    public void setup() {
    ...
    public void addTab(TabSpec tabSpec, Class<?> clss, Bundle args) {
        ...
        if (mAttached) {
            // If we are already attached to the window, then check to make sure
            // this tab's fragment is inactive if it exists.  This shouldn't normally happen.
            // drolmen fix, replace detach() with hide()
    ...
    protected void onAttachedToWindow() {
        ...
        for (int i = 0; i < mTabs.size(); i++) {
            ...
            if (tab.fragment != null && !tab.fragment.isDetached()) {
                if (tab.tag.equals(currentTab)) {
                    // The fragment for this tab is already there and active,
                    // and it is what we really want to have as the current tab.  Nothing to do.
                    mLastTab = tab;
                } else {
                    ...
                    on a null object referencecom.w2here.hoho.ui.view.tab.FragmentTabHost.onAttachedToWindow
                     */
        ...
        if (ft != null) {
            /* 讲commit改为 commitAllowingStateLoss, 避免onSaveInstanceState后调用commit造成crash*/
            ft.commitAllowingStateLoss();
            if (mFragmentManager != null) mFragmentManager.executePendingTransactions();
        }
    }
...
//            throw new IllegalStateException("No tab known for tag " + tabId);
        ...
        if (mLastTab != newTab) {
            ...
            if (newTab.fragment == null) {
                if (newTab.clss == null) {
                    ft.show(mLastTab != null ? mLastTab.fragment : null);
                    return ft;
                }
                newTab.fragment = Fragment.instantiate(mContext,
                        newTab.clss.getName(), newTab.args);
                ft.add(mContainerId, newTab.fragment, newTab.tag);
                findViewById(mContainerId).setContentDescription("add fragment to this container");
            } else {
