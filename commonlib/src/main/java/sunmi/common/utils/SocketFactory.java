...
public class SocketFactory extends javax.net.ssl.SSLSocketFactory {
    ...
    public SocketFactory(SocketFactoryOptions options) throws KeyStoreException, NoSuchAlgorithmException, IOException, KeyManagementException, java.security.cert.CertificateException, UnrecoverableKeyException {
        ...
        if (options.hasCaCrt()) {
            ...
            LogCat.v("Certificate Owner: %s", ca.getSubjectDN().toString());
            LogCat.v("Certificate Issuer: %s", ca.getIssuerDN().toString());
            LogCat.v("Certificate Serial Number: %s", ca.getSerialNumber().toString());
            LogCat.v("Certificate Algorithm: %s", ca.getSigAlgName());
            LogCat.v("Certificate Version: %s", ca.getVersion() + "");
            LogCat.v("Certificate OID: %s", ca.getSigAlgOID());
            Enumeration<String> aliasesCA = caKeyStore.aliases();
            for (; aliasesCA.hasMoreElements(); ) {
                String o = aliasesCA.nextElement();
                LogCat.v("sf", String.format("Alias: %s isKeyEntry:%s isCertificateEntry:%s",
                        o, caKeyStore.isKeyEntry(o), caKeyStore.isCertificateEntry(o)));
            }
        } else {
            LogCat.v("sf", "CA sideload: false, using system keystore");
        ...
        if (options.hasClientP12Crt()) {
            //Log.i("TAG", "MQTT_CONNECTION_OPTIONS.hasClientP12Crt(): true");

            KeyStore clientKeyStore = KeyStore.getInstance("PKCS12");

            clientKeyStore.load(options.getCaClientP12InputStream(),
                    options.hasClientP12Password() ? options.getCaClientP12Password().toCharArray() : new char[0]);
            ...
            //Log.i("TAG", "Client .p12 sideload: false, using null CLIENT cert");
            kmf.init(null, null);
        }
